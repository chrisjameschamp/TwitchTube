...
class twitch:
    ...
    def getCredentials(self):
        creds = util.getFile(constants.TWITCH_CREDS_FILE)
        ...
        util.saveFile(constants.TWITCH_CREDS_FILE, creds)

    def enterCredentials(self, creds):
        if creds is not None and 'client_id' in creds:
            self.client_id = creds['client_id']
        else:
            self.client_id = util.query('Required', 'Twitch Client ID: ', prePrint='Enter the Twitch Client ID for you app.  If you do not have a Client ID visit https://dev.twitch.tv/docs/api')

        if creds is not None and 'oauth' in creds:
            self.oauth = creds['oauth']
        else:
            self.oauth = 'Bearer '+util.query('Required', 'Twitch Oauth Code: Bearer ', prePrint='Enter the Twitch Oauth code.  If you do not have an Oauth code visit https://twitchapps.com/tokengen/')
        ...
        if creds is not None and 'channel' in creds:
            ...
            self.channel = util.query('Required', 'Channel Name: ', prePrint='Enter the Channel Name you would like to download videos from')
    ...
    def getVideos(self):
        ...
        user_input = util.query('Y/N', 'Would you like to keep using that Channel (Y/n)? ', default='Y')
        ...
        for video in videos['data']:
            if video['type'] == 'highlight':
                ...
                highlights.append({'title': video['title'], 'url': video['url'], 'duration': util.seconds(video['duration']), 'filename': video['user_login']+'_'+str(int(dt.timestamp()))+'.mp4'})
        ...
        user_input = util.query('Numeric', 'Video Number: ', min=1, max=count+1)
        if int(user_input) == count+1:
            while True:
                user_input = util.query('Required', 'Enter the URL for the video you would like to use: ')
                twitch_id = util.parseTwitchUrl(user_input)
                video = self.getTwitchData(constants.TWITCH_VIDEO+twitch_id)

                if 'data' in video and video['data']:
                    video = video['data'][0]
                    video = {'title': video['title'], 'url': video['url'], 'duration': util.seconds(video['duration']), 'filename': video['user_login']+'_'+str(int(dt.timestamp()))+'.mp4'}
                    print('We found a video that matches\n'+video['title'])
                    user_input = util.query('Y/N', 'Is that the video you were looking for (Y/n)? ', default='Y')
